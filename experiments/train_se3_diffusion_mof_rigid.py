...
from data import mof_dataset_rigid
...
class Experiment:
    ...
    def __init__(
        ...
        self._model = score_network_mof_rigid.ScoreNetwork(
    ...
    def create_dataset(self):
        ...
        valid_dataset = mof_dataset_rigid.MOFDataset(
        ...
        if not self._use_ddp:
            train_sampler = mof_dataset_rigid.TrainSampler(
                data_conf=self._data_conf,
            ...
            train_sampler = mof_dataset_rigid.DistributedTrainSampler(
                data_conf=self._data_conf,
                dataset=train_dataset,
                batch_size=self._exp_conf.batch_size,
            )
    ...
    def loss_fn(self, batch):
        ...
        trans_loss = torch.zeros(batch_size).to(gt_rot_score.device)
...
@hydra.main(version_base=None, config_path=str(PROJECT_ROOT / "config"), config_name="mof_rigid")
